(ns applianceshop.store-contract-test
  ...
  (doseq [[label s] (backends)]
    (testing label
      (testing "partial upsert merges, preserving untouched fields"
        ...
        (is (= "Sakura Tanaka" (:customer (store/ticket s "ticket-1"))))
        (is (= 45.0 (:claimed-parts-cost (store/ticket s "ticket-1"))) "unrelated field preserved"))
      (testing "assessment / safety-screening / refrigerant-screening payloads commit and read back"
        ...
        (store/commit-record! s {:effect :refrigerant-screening/set :path ["ticket-1"]
                                 :payload {:ticket-id "ticket-1" :certified? true}})
        (is (= {:ticket-id "ticket-1" :certified? true} (store/refrigerant-screening-of s "ticket-1"))))
      ...
      (testing "appliance return drafts a record and advances the return sequence"
        ...
        (is (= "appliance-return-draft" (get (first (store/return-history s)) "kind")))
        (is (true? (:appliance-returned? (store/ticket s "ticket-1"))))
...
(deftest datomic-empty-store-is-usable
  (let [s (store/datomic-store)]
    ...
    (is (zero? (store/next-completion-sequence s "JPN")))
    (is (zero? (store/next-return-sequence s "JPN")))
    (store/with-tickets s {"x" {:id "x" :customer "n" :appliance "d" :appliance-type :washing-machine
