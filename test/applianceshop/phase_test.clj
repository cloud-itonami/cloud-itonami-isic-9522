(ns applianceshop.phase-test
  "The phase table as executable tests. The invariant this repo cannot
  regress on: `:repair/complete`/`:appliance/return` must NEVER be a
...
(deftest appliance-return-never-auto-at-any-phase
  (testing "structural invariant: no phase, now or in the future entries, auto-commits a real appliance return"
    (doseq [[n {:keys [auto]}] phase/phases]
      (is (not (contains? auto :appliance/return))
          (str "phase " n " must not auto-commit :appliance/return")))))

(deftest safety-and-refrigerant-screen-never-auto-at-any-phase
  (testing "screening carries no direct capital risk, but is still never auto-eligible, matching every sibling screening op in this fleet"
    (doseq [[n {:keys [auto]}] phase/phases]
      ...
      (is (not (contains? auto :refrigerant/screen))
          (str "phase " n " must not auto-commit :refrigerant/screen")))))
...
(deftest gate-escalates-a-clean-non-auto-write
  ...
  (is (= :escalate (:disposition (phase/gate 3 {:op :appliance/return} :commit)))))
