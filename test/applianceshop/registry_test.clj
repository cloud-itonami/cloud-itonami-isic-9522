(ns applianceshop.registry-test
  (:require [clojure.test :refer [deftest is]]
            [applianceshop.registry :as r]))

;; ----------------------------- parts-cost-matches-claim? -----------------------------

(deftest matches-when-claim-equals-recompute
  (is (r/parts-cost-matches-claim?
       {:parts-quantity 1 :parts-unit-price 80 :claimed-parts-cost 80.0})))
...
;; ----------------------------- register-appliance-return -----------------------------

(deftest return-is-a-draft-not-a-real-return
  (let [result (r/register-appliance-return "ticket-1" "JPN" 0)]
    (is (nil? (get-in result ["certificate" "proof"])))
    (is (= (get-in result ["certificate" "issued_by_registry"]) false))
    (is (= (get-in result ["certificate" "status"]) "draft-unsigned"))))

(deftest return-assigns-return-number
  (let [result (r/register-appliance-return "ticket-1" "JPN" 7)]
    (is (= (get result "return_number") "JPN-RTN-000007"))
    (is (= (get-in result ["record" "ticket_id"]) "ticket-1"))
    (is (= (get-in result ["record" "kind"]) "appliance-return-draft"))
    (is (= (get-in result ["record" "immutable"]) true))))

(deftest return-validation-rules
  (is (thrown? Exception (r/register-appliance-return "" "JPN" 0)))
  (is (thrown? Exception (r/register-appliance-return "ticket-1" "" 0)))
  (is (thrown? Exception (r/register-appliance-return "ticket-1" "JPN" -1))))
