(ns applianceshop.repairopsllm
  "RepairOps-LLM client -- the *contained intelligence node* for the
  household-appliance-repair actor.

  It normalizes ticket intake, drafts a per-jurisdiction consumer-
  product-safety/refrigerant-handling evidence checklist, screens
  tickets for a failed post-repair safety test, screens tickets for an
  unconfirmed refrigerant-handling certification, drafts the repair-
  completion action, and drafts the appliance-return action.
  CRITICAL: it is a smart-but-untrusted advisor. It returns a
  *proposal* (with a rationale + the fields it cited), never a
  committed record or a real repair completion/appliance return. Every
  output is censored downstream by `applianceshop.governor` before
  anything touches the SSoT, and `:repair/complete`/`:appliance/
  return` proposals NEVER auto-commit at any phase -- see README
  `Actuation`.
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     ...
     :stake      kw|nil         ; :actuation/complete-repair | :actuation/return-appliance | nil
     :confidence 0..1}"
...
(defn- normalize-intake
  ...
  does not invent the customer, appliance/parts costs or jurisdiction.
...
(defn- assess-jurisdiction
  "Per-jurisdiction consumer-product-safety/refrigerant-handling
  ...
  NO official spec-basis in `applianceshop.facts` -- the Repair Shop
  ...
  (let [t (store/ticket db subject)
    ...
    (if (nil? sb)
      {:summary    (str iso3 " の公式spec-basisが見つかりません")
       :rationale  "applianceshop.facts に未登録の法域。要件を推測で作らない。"
       :cites      []
...
(defn- screen-refrigerant
  "Refrigerant-handling-certification screening draft.
  `:refrigerant-handling-certified?` on the ticket record injects the
  ...
  (let [t (store/ticket db subject)]
    (cond
      (nil? t)
      {:summary "対象ticketが見つかりません" :rationale "no ticket record"
       :cites [] :effect :refrigerant-screening/set :value {:ticket-id subject :certified? false}
       :stake nil :confidence 0.0}

      (false? (:refrigerant-handling-certified? t))
      {:summary    (str (:customer t) ": フロン類取扱技術者資格の未確認を検出")
       :rationale  "スクリーニングがフロン類取扱資格の未確認を検出。人手確認とホールドが必須。"
       :cites      [:refrigerant-check]
       :effect     :refrigerant-screening/set
       :value      {:ticket-id subject :certified? false}
       :stake      nil
       :confidence 0.95}

      :else
      {:summary    (str (:customer t) ": フロン類取扱資格を確認")
       :rationale  "フロン類取扱資格スクリーニング完了。"
       :cites      [:refrigerant-check]
       :effect     :refrigerant-screening/set
       :value      {:ticket-id subject :certified? true}
       :stake      nil
       :confidence 0.9})))
...
(defn- propose-repair-completion
  ...
  a phase's `:auto` set (`applianceshop.phase`); the governor also
  ...
  (let [t (store/ticket db subject)
        matches? (and t (registry/parts-cost-matches-claim? t)
                     (:refrigerant-handling-certified? t))]
    {:summary    (str subject " 向け修理完了提案"
     ...
     :rationale  (if t
                   (str "claimed-parts-cost=" (:claimed-parts-cost t)
                        ...
                        " refrigerant-handling-certified?=" (:refrigerant-handling-certified? t))
                   "ticketが見つかりません")
     :cites      (if t [subject] [])
...
(defn- propose-appliance-return
  "Draft the actual APPLIANCE-RETURN action -- returning a real
  appliance to the customer. ALWAYS `:stake :actuation/return-
  appliance` -- this is a REAL-WORLD act (a potentially unsafe
  appliance, or one serviced without confirmed refrigerant-handling
  certification, could reach a customer), never a draft the actor may
  auto-run. See README `Actuation`: no phase ever adds this op to a
  phase's `:auto` set (`applianceshop.phase`); the governor also
  always escalates on `:actuation/return-appliance`. Two independent
  layers agree, deliberately."
  [db {:keys [subject]}]
  (let [t (store/ticket db subject)
        ready? (and t (:safety-test-passed? t) (:refrigerant-handling-certified? t))]
    {:summary    (str subject " 向け返却提案"
     ...
     :value      {:ticket-id subject}
     :stake      :actuation/return-appliance
...
(defn infer
  ...
  (case op
    ...
    :refrigerant/screen                     (screen-refrigerant db request)
    :repair/complete                            (propose-repair-completion db request)
    :appliance/return                               (propose-appliance-return db request)
...
(def ^:private system-prompt
  (str "あなたは家電修理店の修理完了・返却エージェントの助言者です。"
       "与えられた事実のみに基づき、提案を1つだけEDNマップで返します。説明や前置きは"
       "一切書かず、EDNだけを出力します。\n"
       "キー: :summary(人向けドラフト) :rationale(根拠/必ず事実から) "
       ":cites(使った事実キーのベクタ) "
       ":effect(:ticket/upsert|:assessment/set|:safety-screening/set|"
       ":refrigerant-screening/set|:ticket/mark-completed|:ticket/mark-returned) "
       ":stake(:actuation/complete-repair か :actuation/return-appliance か nil) :confidence(0..1)。\n"
...
(defn- facts-for [st {:keys [op subject]}]
  (case op
    ...
    :refrigerant/screen   {:ticket (store/ticket st subject)}
    :repair/complete      {:ticket (store/ticket st subject)}
    :appliance/return     {:ticket (store/ticket st subject)}
