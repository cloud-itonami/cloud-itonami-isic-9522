(ns applianceshop.operation
  "OperationActor -- one household-appliance-repair operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (RepairOps-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Repair Shop Governor (:govern)
  ...
  One graph run = one household-appliance-repair operation (intake ->
  ...
  decision to a human operator (repair technician). The approver
  resumes with `{:approval {:status :approved}}` (or :rejected).
  `:repair/complete`/`:appliance/return` ALWAYS reach this node when
  the governor is clean -- see `applianceshop.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [applianceshop.repairopsllm :as repairopsllm]
            [applianceshop.governor :as governor]
            [applianceshop.phase :as phase]
...
(defn build
  ...
  `applianceshop.store/Store`).
  opts:
    :advisor      -- a `applianceshop.repairopsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (repairopsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; RepairOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (repairopsllm/-advise advisor store request)]
            {:proposal p :audit [(repairopsllm/trace request p)]})))

      ;; Repair Shop Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
